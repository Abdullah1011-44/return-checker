...
export default function Dashboard() {
  ...
  const themeEditorUrl = useMemo(
    () => buildShopifyThemeEditorUrl(shopDomain),
    [shopDomain],
  );
  ...
  async function performAction(request, action, merchantNote) {
    ...
    try {
      ...
      if (IS_DEV) {
        console.info("[Dashboard:action-response]", {
          returnRequestId: request.id,
          action,
          success: data.success,
          emailSent: data.emailSent ?? null,
          notificationSent: data.notificationSent ?? null,
      ...
      const emailFailedWithWarning =
        !emailWasSent && Boolean(data.email?.error);
      ...
      } else if (emailFailedWithWarning) {
        setActionEmailFeedback((prev) => ({
          ...prev,
          [request.id]: "warning",
        }));
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">
                Add Return Assistant to your store
              </h2>
              <p className="text-xs text-slate-500 mt-1 leading-relaxed max-w-xl">
                Enable the Return Assistant from your Shopify theme editor so
                customers can start returns directly from your storefront.
              </p>
              {!themeEditorUrl && (
                <p className="text-xs text-amber-700 mt-2">
                  Connect Shopify to open the theme editor from here.
                </p>
              )}
            </div>
            {themeEditorUrl
              ? <a
                  ...
                  className="inline-flex shrink-0 items-center justify-center text-xs font-semibold text-white bg-slate-900 hover:bg-slate-800 rounded-full px-4 py-2 shadow-sm transition-all duration-150"
                >
                  Open Theme Editor
                </a>
              : <button
                  type="button"
                  disabled
                  className="inline-flex shrink-0 items-center justify-center text-xs font-semibold text-slate-400 bg-slate-100 border border-slate-200 rounded-full px-4 py-2 cursor-not-allowed"
                >
                  Open Theme Editor
                </button>}
          </div>
