...
export async function GET(request) {
  ...
  try {
    ...
    const analytics = await loadMerchantRecoveryAnalytics(prisma, merchant.id, {
      range: rangeResult.range,
    });

    const { queryWindow: _queryWindow, ...payload } = analytics;
    ...
    captureException(error, {
      ...
      action: "dashboard_recovery_analytics",
    });

    console.error("[GET /api/dashboard/recovery]", error);

    return NextResponse.json(
      { success: false, message: "Failed to load recovery analytics." },
      { status: 500 },
    );
  }
}
